...
function getArrayListtestimonials(){ /* spertinya perlu lock table*/ 
 $xBuffResul = array(); 
 $xStr =  "SELECT ".
      "idx".",coupon_id".
",coupon_number".
...
function getLastIndextestimonials(){ /* spertinya perlu lock table*/ 
 $xStr =   "SELECT ".
      "idx".
      ",coupon_id".
",coupon_number".
...
{
 $xidpegawai = $this->session->userdata('idpegawai');    $xStr="insert into logdelrecord(idxhapus,nmtable,tgllog,ideksekusi) values($xidx,'testimonials',now(),$xidpegawai)"; 
    $query = $this->db->query($xStr);
}
